...
void csc_matrix_vector_sse(CSCMatrix A, float *Output_vector, float *Value_vector, float *val) {
    for (int j = 0; j < A.rows; j++) {
        __m128 sum = _mm_setzero_ps();
        int i;
        for (i = A.column_ptrs[j]; i <= A.column_ptrs[j + 1] - 4; i += 4) {
            __m128 val_vec = _mm_loadu_ps(&Value_vector[i]);
            __m128 vec_val = _mm_loadu_ps(&val[i]);
            __m128 prod = _mm_mul_ps(val_vec, vec_val);
...
int main(int argc, char *argv[]){
    ...
    csc_matrix_vector_sse(A, Output_vector, vector_val, val_float);
