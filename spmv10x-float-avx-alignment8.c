...
void csc_matrix_vector_avx_aligned8(CSCMatrix A, float *Output_vector, float *Value_vector, float *val) {
    for (int j = 0; j < A.rows; j++) {
        ...
        if (remaining > 0) {
            float temp_values[8] = {0};
            float temp_val[8] = {0};
            int temp_indices[8] = {0};
            
            for (int k = 0; k < remaining; k++) {
                temp_values[k] = Value_vector[i + k];
                temp_val[k] = val[i + k];
                temp_indices[k] = A.row_indices[i + k];
            }
            
            __m256 val_vec = _mm256_loadu_ps(temp_values);
            __m256 vec_val = _mm256_loadu_ps(temp_val);
            __m256 prod = _mm256_mul_ps(val_vec, vec_val);
            
            float result[8];
            _mm256_storeu_ps(result, prod);
            
            for (int k = 0; k < remaining; k++) {
                Output_vector[temp_indices[k]] += result[k];
            }
        }
    }
}
...
int main(int argc, char *argv[]){
    ...
    csc_matrix_vector_avx_aligned8(A, Output_vector, vector_val, val_float);
