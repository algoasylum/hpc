...
void csc_matrix_vector_product_sse(CSCMatrix A, float *x, float *y) {
    for (int j = 0; j < A.cols; j++) {
        __m128 x_vec = _mm_set1_ps(x[j]);
        for (int i = A.column_ptrs[j]; i < A.column_ptrs[j + 1]; i += 4) {
            int remaining = A.column_ptrs[j + 1] - i;
            if (remaining >= 4) {
                __m128 val_vec = _mm_loadu_ps(&A.values[i]);
                __m128 row_vec = _mm_loadu_ps(&y[A.row_indices[i]]);
                __m128 result = _mm_add_ps(row_vec, _mm_mul_ps(val_vec, x_vec));
                _mm_storeu_ps(&y[A.row_indices[i]], result);
            } else {
