...
/*void csc_matrix_vector(CSCMatrix A, float *Output_vector, float *Value_vector, float *val){
    //float val_spec=0;
    //#pragma omp parallel for reduction(+:Output_vector[:A.rows])
    #pragma omp parallel for schedule(dynamic) num_threads(4)
...
int main(int argc, char *argv[]){
    ...
    float elapsed_time = elapsed_time1+elapsed_time2;
    printf("Time elaptsed TOTAL: %f seconds\n", elapsed_time);
...
//>gcc -std=c99 spmv-float-10X-OPENMP.c -o SPMV-10X-F-OPENMP.exe -fopenmp mmio.c
