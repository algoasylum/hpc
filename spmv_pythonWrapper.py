...
def run_spmv(matrix_file):
    result = subprocess.run(["./SPMV-DOUBLE", matrix_file], capture_output=True, text=True)
    if result.returncode != 0:
        print(f"Error running spmv for {matrix_file}: {result.stderr}")
        return None, None, None, None
    else:
        # Extract the average execution time, rows, columns, and non-zero values from the output
        exec_time = None
        rows, columns, non_zero = None, None, None
        for line in result.stdout.splitlines():
            if " time required to perform normal spmv with same input vector in line":
...
if __name__ == "__main__":
    ...
    for i, (matrix_file, data) in enumerate(results.items(), 1):
        ...
        print(f"  Execution time: {data['exec_time']} seconds")
