...
void csc_matrix_vector_product_sse(CSCMatrix A, double *x, double *y) {
    for (int j = 0; j < A.cols; j++) {
        __m128d x_vec = _mm_set1_pd(x[j]);
        for (int i = A.column_ptrs[j]; i < A.column_ptrs[j + 1]; i += 2) {
            int remaining = A.column_ptrs[j + 1] - i;
            if (remaining >= 2) {
                __m128d val_vec = _mm_loadu_pd(&A.values[i]);
                __m128d row_vec = _mm_set_pd(y[A.row_indices[i+1]], y[A.row_indices[i]]);
                __m128d result = _mm_add_pd(row_vec, _mm_mul_pd(val_vec, x_vec));
...
int main(int argc, char *argv[]) {
    ...
    csc_matrix_vector_product_sse(A, Input_vector, Output_vector);
