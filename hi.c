    ...
    for (int j = 0; j < A.cols; j++) {
        ...
        for (int i = A.column_ptrs[j]; i < A.column_ptrs[j + 1]; i += 4) {
            ...
            if (remaining >= 4) {
                ...
                __m128 row_vec = _mm_set_ps(y[A.row_indices[i+3]], y[A.row_indices[i+2]],
