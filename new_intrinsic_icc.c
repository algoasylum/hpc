...
void csc_matrix_vector_avx(CSCMatrix A, double *Output_vector, double *Value_vector, double *val) {
    for (int j = 0; j < A.rows; j++) {
        ...
        for (int i = A.column_ptrs[j]; i < A.column_ptrs[j + 1]; i++) {
            __m256d vec_value_vector = _mm256_set1_pd(Value_vector[i]); // Load scalar Value_vector[i] into SIMD register
            __m256d vec_val = _mm256_set1_pd(val[i]); // Load scalar val[i] into SIMD register
...
//icx-cl /FeNEW-INTRINSIC-ICC.exe new_intrinsic_icc.c /QxAVX /O2 /link /LIBPATH:"C:\Program Files (x86)\Intel\oneAPI\compiler\latest\lib"

//#exec_name3="C:/Users/DELL/OneDrive/Documents/samplesss./NEW-INTRINSIC-ICC.exe"